...
#### 0. 环境约定补齐（2026-06-22）
- ✅ 明确当前开发机为 macOS
  - Mac 负责代码逻辑、文档、静态检查、mock 测试和轻量验证
  - Mac 不承担 CUDA/GPU 训练、pydirectinput、真实游戏窗口和端到端推理验证
- ✅ 明确训练与真实验证环境为 Windows 10 + RTX 5060 8GB
  - Windows 10 负责完整依赖、CUDA/GPU 训练、真实输入模拟、游戏窗口截屏和性能测试
- ✅ 更新项目规则和 README，后续计划需区分「Mac 可验证」和「Windows 10 必验证」
...
#### 阶段3.5.5：集成测试（0.5天）✅ 部分完成
    ...
    **完成内容**：
    ...
    #### 阶段3.5.6：在线学习系统（后续优化⏸️）
    - [ ] 实现在线数据收集（backend/ai/finetune/online_collector.py）
      - [ ] 真实对局记录（状态、动作、奖励）
      - [ ] 在线经验回放（与离线buffer整合）
      - [ ] 对手风格建模（激进/保守/中等）
      - [ ] 自动筛选高质量对局
    - [ ] 实现在线DQN微调（backend/ai/finetune/online_trainer.py）
      - [ ] 离线预训练模型加载
      - [ ] 实时经验回放更新
      - [ ] 在线TD更新
      - [ ] 模型定期保存（每局保存一次）
...
由于大乱斗模式英雄随机分配，无法保证获得指定英雄的录像，因此采用**混合策略（方案C）**：
- 阶段1：V1专注2-3个常用英雄训练专精模型
- 阶段2：V2+扩展到多英雄泛化模型
- 阶段3：在线学习优化

### 详细规划

**完整文档**：[混合策略数据收集与训练方案](mixed_strategy_training_plan.md)
...
| 方案 | 周期 | 局数 | 成果 |
|------|------|------|------|
| **仅阶段0-1（推荐）** | 23-28天 | 40-55局 | V1：支持2-3个英雄 |
| **阶段0-2（完整）** | 50-60天 | 90-130局 | V2：支持多英雄类型 |
...
- 功能完成需自检
 ...
 5. ~~训练稳定性问题~~（经验回放解决）
 6. **缺乏足够的高手录像资源**（2026-01-21）
    - 问题：高手录像不公开，难以获取
    - 解决方案：使用自己录制的游戏录像（一般水平）
    - 策略：
      - 专注2-3个常用英雄，降低数据需求
      - 通过数据筛选和质量控制保证数据可用性
      - 利用在线学习持续优化
 7. **大乱斗模式英雄随机分配**（2026-01-21）
    - 问题：无法保证录制到指定英雄的录像
    - 解决方案：采用混合策略（方案C）
    - 详细文档：[混合策略数据收集与训练方案](mixed_strategy_training_plan.md)
    - 策略：
      - 阶段1（V1）：专注2-3个常用英雄训练专精模型
      - 阶段2（V2）：扩展到多英雄泛化模型
      - 阶段3（V3+）：在线学习优化
 ...
 **创建日期**: 2026-01-20
 **最后更新**: 2026-01-21
 **当前状态**: 阶段3.5大部分已完成，等待数据收集后进行训练和测试；已规划在线学习系统作为后续优化方向；采用混合策略解决大乱斗英雄随机分配问题
